...
public class IglooCache {
	...
	/* ip 대역을 첫번째, 두번째로 나누어 Hash Table로 데이터를 가지고있는다.*/
	...
	public boolean addUser(UserInfo user){
		...
		userKeys.put(user.getMac(), keyList);
	...
	public boolean updateUser(UserInfo user) {
		...
		List<Integer> user_keys = userKeys.get(key);
		
		if(user_keys == null || user_keys.size() == 0){
			return false;
		}
		...
		UserInfo old_user = userMap.get(hash_key).get(user_key);
		
		if(user.getEmail() != null){
			userKeys.remove(old_user.getEmail());
			userKeys.put(user.getEmail(), user_keys);
			old_user.setEmail(user.getEmail());
		}
		if(user.getName() != null){
			userKeys.remove(old_user.getName());
			userKeys.put(user.getName(), user_keys);
			old_user.setName(user.getName());
		}
		if(user.getIp() != null){
			userKeys.remove(old_user.getIp());
			userKeys.put(user.getIp(), user_keys);
			old_user.setIp(user.getIp());
		}
		if(user.getMac() != null){
			userKeys.remove(old_user.getMac());
	...
	public boolean removeUser(String userId){
		...
		if(userKeys.get(userInfo.getMac()) != null){
			userKeys.remove(userInfo.getMac());
		}
		return true;
	}

	public boolean removeUserPCInfo(UserInfo user) {
		if(user.getIp() == null || user.getMac() == null){
			return false;
		}
		
		String ip = user.getIp();
		String mac = user.getMac();
		List<Integer> user_keys = userKeys.get(ip);
		if(user_keys.size() == 0){
			return false;
		}
		...
		if(userMap.get(hash_key).get(user_key) != null){
			userMap.get(hash_key).get(user_key).setIp(null);
			userMap.get(hash_key).get(user_key).setMac(null);
			userKeys.remove(ip);
			userKeys.remove(mac);
			return true;
		}
	...
	public String getMac(String value) {
		...
		if(userMap.get(hash_key) == null){
			...
			return userMap.get(hash_key).get(user_key).getMac();
		}
	}
